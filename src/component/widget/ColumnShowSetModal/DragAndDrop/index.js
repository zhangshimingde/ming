...
class DragAndDrop extends Component {
  ...
  move = (from, to)=>{
    if(from === "操作" || to === "操作"){ message.warning('操作列不可移动'); return ;}
    if(from === "序号" || to === "序号"){ message.warning('序号列不可移动'); return ;}
    const {handleSetShowColumns,showColumns,allColumns} = this.props;
    // find items
    ...
    handleSetShowColumns(newNextLayout,allColumns);
  }
